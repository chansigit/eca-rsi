...
def test_groups_collapsed_with_tallies_and_species(tmp_path):
    ...
    assert "2 done · 2 working · " in html and ">1 failed</span>" in html
...
def test_group_tally_drops_zero_parts():
    assert serve.group_tally({"released": 3}) == "3 done"
    assert serve.group_tally({"running": 1, "neutral": 1}) == "2 working"
