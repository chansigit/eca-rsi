...
from ecarsi.persample_workflow import saved_module


def test_saved_module_keeps_v3_and_defaults_new(tmp_path):
    folder = tmp_path / 'requests' / 'run.agent-1'
    folder.mkdir(parents=True)
    (folder / 'request.json').write_text(json.dumps({'spec': {'args': ['-m', 'ecarsi.zoomin_v3', 'agent', 'x']}}))
    assert saved_module(tmp_path, 'run.agent-1') == 'ecarsi.zoomin_v3'
