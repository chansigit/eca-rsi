...
MODULES = {'ecarsi.persample_v2', 'ecarsi.crosssample_v2', 'ecarsi.zoomin_v2'}
...
def plan(request, directory, session):
    ...
    from .agent_tool_execution import plan as original_plan
    ...
    wrapped = dict(request, args=['-m', 'ecarsi.agent_evidence', packet['path']],
    ...
    if args[1] == 'ecarsi.persample_v2' and read(args[4])['version'] == 0:
...
def next_required(module, state, allowed, multimodal):
    ...
    if module == 'ecarsi.persample_v2':
        from .persample_v2 import evidence_files
...
def execute(packet_path):
    ...
    for index in range(MAX_CALLS):
        ...
        if (packet['module'] == 'ecarsi.crosssample_v2' and name == 'read_evidence'
