...
def adapter_path(is_turn):
    """Host code a request pins by hash: the session adapter for model turns, the plan validator otherwise."""
    return Path(__file__).with_name("session.py") if is_turn else Path(__file__).parents[1] / "plan.py"
...
def retry_turn(root, request_id, *, reason):
    ...
    from .dispatch import policy, credential_timeout, invalid_dispatch_snapshot
...
def execute(root, request_id):
    ...
    with lock(folder / "execution.lock", blocking=False):
        ...
        try:
            ...
            if file_digest(adapter_path(is_turn)) != request["adapter_sha256"]:
