...
def choose(session_ref, reply_path):
    from .agent_session import turn_reply
...
def budget(request, directory, spec, tool, state_ref):
    ...
    if request['args'][:2] == ['-m', 'ecarsi.agent_evidence']:
        # Existing evidence batches may also execute QC, which loads the matrix.
        light = light and args[1] != 'ecarsi.persample_v2' and state.get('phase') != 'quality' and state.get('kind') != 'lineage'
    if light:
        optimized = dict(request, memory_mb=min(request['memory_mb'], 2048))
    elif args[1] != 'ecarsi.persample_v2' and tool['name'] in READS:
