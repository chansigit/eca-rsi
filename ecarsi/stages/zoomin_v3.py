...
def agent_spec(spec, evidence, kind, parent):
    ...
    checklist = PROMPTS / ('zoomin-%s-checklist-v4.md' % ('plan' if kind == 'plan' else 'annotation'))
    session['prompt'] += '\n\n' + checklist.read_text()
    tools = []
...
def canonical_arguments(name, args):
    ...
    if name in SUBMISSIONS and isinstance(args.get('proposal_json'), str):
        try:
