...
```bash
python -m ecarsi.control --temporal HOST:7233 start-zoomin spec.json
python -m ecarsi.control --temporal HOST:7233 status-zoomin RUN_ID
python -m ecarsi.control --temporal HOST:7233 resume-zoomin RUN_ID
```
...
`python -m ecarsi.control --service-root /absolute/control set-deg-limit zoom-in RUN_ID 16`.
...
Uterus exercised the CPU route while Prostate agents waited: three lineage numerical units completed, with the first two running concurrently. Its complete workflow retained 244/291 cells and excluded 47. Stromal retained 95/136; T Lymphocyte retained 69/71; Myeloid retained 67/71. Another 13 cells in skipped lineages retained their previous labels. Six T-lineage cells were reassigned and remained survivors, verified separately from exclusions. Stromal's above-budget proposal underwent a second removal review. The same final artifact, cell identity and dual-resolution checks passed. The small acceptance runs explicitly used `min_cells=50` instead of changing the production default.

`zoomin-numerical-acceptance.json` holds the five computation receipts. `zoomin-final-acceptance.json` records both completed publication checks. Publication SHA-256 values are `4ceeb87049bb1038cbfefd122a3070195f924d7f8b027cfaf8fe320821c10ac5` (Prostate) and `e4bc87a091cf0c8aa2353cd0df9ca3ff4a2713898f39b5c59091ca7f83f2a0fc` (Uterus).

Acceptance exposed two avoidable sources of model round trips. Subsequent prompt guidance documents batch marker queries; guard errors now name only the missing required checks. The shared MSP evidence reader also preserves completed empty DEG tables in SQLite metadata and distinguishes empty results from missing computations. Eight targeted checks covered the evidence fix, including a test against the actual updated container package. These changes do not alter the accepted numerical results or rewrite the original serialized prompts. Source files pinned by unfinished sessions must remain unchanged until those sessions finish or a deliberate version migration is performed.

After acceptance, both development workers registered runtime `a95403c9820a80918102d0dd4516c844de83c5ac9cf9df13dca9c9e4f654c1d6` from `rsi-science-20260915-6.sif`. Pool task `empty-deg-runtime-v6-20260915` then verified the corrected immutable database on an actual worker. The final lineage regression passed, and 25 saved Temporal histories, including both completed zoom-in workflows, replayed against the current definitions. The accepted scientific publications retain their original image/version records; old bulk production remains paused.
